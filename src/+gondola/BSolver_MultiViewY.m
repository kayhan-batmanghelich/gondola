...
%               s.t.        B => 0, C => 0
%                           B >= 1
%                           1^t  B(m)_i   <= lambda_const
%
%       where:
%               D(V;BC)   :    lambda_gen*\sum_m || V(m) - B(m)*C(m) ||_{F}^{2} 
%               l(y;X,B,w):    \sum_{k=1}^{K}  \lambda_disc*\eta_k *\sum_{i \in A_k}{max{ 0, \sum_m  1_{y_i=k}  < w(m,k) , B(m)^t X_i(m) >  }^2} + \lambda_disc*\sum_{i \notin A_k}{max{ 0, \sum_m  -1_{y_i~=k}  < w(m,k) , B(m)^t X_i(m) >  }^2} 
%
...
function [B,Report]  = BSolver_MultiViewY(B0,V,C,w,y,options)
    % decomposing options and creating necessary variables
    D = size(B0,1) ;
    r = size(B0,2) ;
    numChannels = size(V,3) ;
    ...
    for ii=1:numChannels
        ...
        f = @(x)obj_fun(x,ii,V,B0,C,w,y,options) ;
        g = @(x)obj_gradient(x,ii,V,B0,C,w,y,options) ;
        projFcn = @(x)ProjectionBasisVectors(x,lb, ub,lambda_const,D,r) ;
        x0  = projFcn( reshape(B0(:,:,ii),[D*r,1]) ) ;
        sol = gondola.spg(x0, f, g, BBMethod, projFcn, lb, ub) ;
        B(:,:,ii) = reshape(sol.par,[D,r]) ;
        B0(:,:,ii) = B(:,:,ii) ;
    end
...
% objective function
function f = obj_fun(B,idx,V,B0,C,w,y,options)
    ...
    tmpB = B ;
    B = B0 ;
    B(:,:,idx) = reshape(tmpB,[D,r]) ;   clear tmpB
...
% gradient function
function g = obj_gradient(B,idx,V,B0,C,w,y,options)
    lambda_gen = options.lambda_gen ;
    lambda_disc = options.lambda_disc ;
    ...
    g_rec = zeros(D,r) ; 
    g_rec = - 2*lambda_gen*( V(:,:,idx)*C(:,:,idx)' - B(:,:,idx)*(C(:,:,idx)*C(:,:,idx)') ) ; 
    ...
    if (numClasses > 2)
      for classCnt=1:numClasses
        ...
        stIdx = 1 + (idx-1)*r ;
        endIdx = idx*r ;
        V1 = V(:,y==classCnt,idx) ;   % class "classCnt"
        V2 = V(:,y~=classCnt,idx) ;   % whoever not in that class
        if isempty(index1)
        ...
        if ~isempty(class_N)   % make sure that there is any labeled data
          ...
          stIdx = 1 + (idx-1)*r ;
          endIdx = idx*r ;
          V1 = V(:,y==1,idx) ;   % class "classCnt"
          V2 = V(:,y~=1,idx) ;   % whoever not in that class
          if isempty(index1)
            g_disc1 = g_disc1 + zeros(size(g_rec)) ;
          else
