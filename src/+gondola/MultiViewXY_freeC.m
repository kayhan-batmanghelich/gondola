...
%
%       Note: Here, C does not have to be positive; in other words, it is FREE. 
function [B,C,w,Report] = MultiViewXY_freeC(Data,ConstVars) 
    ...
    if ~(isfield(Data,'W0') && isfield(Data,'H0') && isfield(Data,'w0') )
        display('Random initialization of blocks (w,B,C) .... ') ; 
        Data.W0 = rand(D,r,'single') ;
        Data.W0 = Data.W0./repmat(sum(Data.W0),D,1)*ConstVars.lambda_const/2 ;   % to make initialization feasible
        Data.H0 = randn(r,N,numChannels,'single') ;
        if (numClasses > 2)
            Data.w0 = randn(r*numChannels,numClasses,'single') ;
        else
            Data.w0 = randn(r*numChannels,1,'single') ;
        end
    end
    ...
    for iter = iter0:MAXITR
        ...
        if mod(iter,3) == 1    % optimization wrt to B
            ...
            B_old = B ;
            B0 = B ;
            [B,Report]  = gondola.BSolver_MultiViewXY(B0,V,C,w,y,ConstVars) ;
            [Obj_Hist, Terms_Hist] = UpdateHistory(Obj_Hist,Terms_Hist,V,B,C,w,y,class_N,ConstVars) ;
        ...
        elseif mod(iter,3) == 2    % optimization wrt to w
            ...
            if (lambda_disc~=0) 
                ...
                for chanCnt=1:numChannels
                    fMat = [fMat  V(:,y~=0,chanCnt)'*B] ;
                end
                fMat = sparse(double(fMat)) ;
            ...
            for chanCnt=1:numChannels
                if strcmpi(ConstVars.csolver, 'mosek')
                    [C(:,:,chanCnt),~]  = gondola.CSolver_freeC_mosek(C0(:,:,chanCnt),V(:,:,chanCnt),B,w,y,ConstVars) ;
                else
                    [C(:,:,chanCnt),~]  = gondola.CSolver_freeC_spg(C0(:,:,chanCnt),V(:,:,chanCnt),B,w,y,ConstVars) ;
                end
            end
            [Obj_Hist, Terms_Hist] = UpdateHistory(Obj_Hist,Terms_Hist,V,B,C,w,y,class_N,ConstVars) ;
...
function [Obj_Hist, Terms_Hist] = UpdateHistory(Obj_Hist,Terms_Hist,V,B,C,w,y,class_N,ConstVars)
    ...
    endIdx = r ;
    for chanCnt=1:numChannels
          %fprintf('computing generative term, vol [%d] ....',chanCnt) ;tic
