% This solver solves the following objective function:
...
function [B,C,w,Report] = MultiViewXY(Data,ConstVars) 
        ...
        elseif mod(iter,3) == 2    % optimization wrt to w
            ...
            for chanCnt=1:numChannels
                if strcmpi(ConstVars.csolver, 'mosek')
                    [C(:,:,chanCnt),Report] = gondola.CSolver_mosek(C0(:,:,chanCnt),V(:,:,chanCnt),B,w,y,ConstVars) ;
                else
                    [C(:,:,chanCnt),Report] = gondola.CSolver_spg(C0(:,:,chanCnt),V(:,:,chanCnt),B,w,y,ConstVars) ;
                end
                [Obj_Hist, Terms_Hist] = UpdateHistory(Obj_Hist,Terms_Hist,V,B,C,w,y,class_N,ConstVars) ;
